...
var ejs = require('ejs');
var asconfdir = '/etc/asterisk/';
var dahcondir = '/etc/dahdi/';
var AsyncConfig = {};
AsyncConfig.sip_exten = function(cb) {

}
AsyncConfig.sip_trunk = function(cb) {
	async.auto({
		//备份文件
		backfile: function(callback) {
			copyFile('filename',asconfdir+'back/', callback);
		},
		//从数据库读取数据
		gettrunks:function(callback){

		},
		//获取模板
		gettpl:function(callback){

		},
		//渲染
		render:['gettpl',function(callback,results){

		}],
		//保存配置到文件
		savefile:['backfile','render',function(callback,results){

		}],
		//重新加载配置文件，最佳方式是重启
		restartAs:['savefile',function(callback,results){
...
AsyncConfig.iax_exten = function(cb) {

}
AsyncConfig.iax2_exten = function(cb) {

}
AsyncConfig.iax2_trunk = function(cb) {

}
AsyncConfig.iax2_trunk = function(cb) {

}
AsyncConfig.queues_list = function(cb) {

}
AsyncConfig.chan_dahdi = function(cb) {

}
AsyncConfig.dahdi_channels = function(cb) {

}
exports.asynconfig = function(req, res, next) {
	var asynctype = req.body['asynctype'];
	if (_.isFunction(AsyncConfig[asynctype])) {
		AsyncConfig[asynctype](function(err, results) {
